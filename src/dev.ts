...
function runAstCodeParser() {
  let code = fs
    .readFileSync("./src/parsers/examples/java8/Java8.ast")
    .toString();
  ...
  for (let ruleBlock of parser.ruleBlocks().ruleBlock()) {
    ...
    let generatedRuleClassName =
      ruleName.charAt(0).toUpperCase() + ruleName.slice(1) + "Context";
    ...
    if (ruleValue.structValue()) {
      ...
      for (let structField of structValue.structField()) {
        ...
        if (fieldValue.structFieldValue_optional()) {
          let fieldRuleName = fieldValue.structFieldValue_optional()!.ruleName()
            .text;
...
`
          ...
          let fieldRuleName = fieldValue.structFieldValue_single()!.ruleName()
            .text;
          structFieldsInitialisationCode.push(
            `let ${fieldName}Context = context.${fieldRuleName}();
struct.${fieldName} = generate_${fieldRuleName}(${fieldName}Context);
`
          ...
          let fieldRuleName = fieldValue.structFieldValue_array()!.ruleName()
            .text;
          structFieldsInitialisationCode.push(
